...
- `migration/bridge_status.ts` — read-only cutover monitor, run on demand: `node --env-file="$MIGRATION_DATA_DIR/.env" migration/bridge_status.ts`. Reports how many anonymous placeholders are still unclaimed (the number that decides when the bridge can be deleted), and exits non-zero on two real misconfigurations: a `kind='user'` tombstone against an unclaimed placeholder (which would make the delta-sync refuse that user's journals), and Firebase's Anonymous provider being re-enabled. Unclaimed placeholders alone are the expected mid-window state and never fail the run. Not wired into `sync.sh` — that runs under `set -e`, so a non-zero exit here would mark a successful sync as failed.
