...
**Text-column overflow guard**: the title/date `Column` is wrapped in `Flexible` (so it absorbs sub-pixel height rounding from the grid's `childAspectRatio: 0.59` cell calculation) and the text styles carry `height: 1.1` to tighten line spacing. Without these, the outer Column overflows by ~1 pixel during the context-menu zoom animation where sizes interpolate frame-by-frame.
